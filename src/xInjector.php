...
class xInjector implements IInjector {
		...
		try {
			...
			if (class_exists($currentClassName)) {
				...
				if ($classInfo->isInterface()) {
					throw new InjectorException("xInjector->instantiate(): interface $className cannot be instantiated");
				}
				if ($classInfo->isAbstract()) {
					throw new InjectorException("xInjector->instantiate(): abstract class $className cannot be instantiated");
				}
				return $classInfo->newInstanceArgs($this->getClassArgs($classInfo, $additionalContext));
			}
		} catch (ReflectionException $ex) {
			throw new InjectorException("xInjector->instantiate(): ReflectionException: ".$ex->getMessage(), $ex->getCode(), $ex);
		}
		throw new InjectorException("xInjector->instantiate(): class $currentClassName not found");
	}
